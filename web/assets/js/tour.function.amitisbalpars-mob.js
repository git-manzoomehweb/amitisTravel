document.getElementById('shareBtn').addEventListener('click', async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'آمیتیس | مجری تخصصی تورهای خارجی، رزرو بلیط، هتل، خدمات ویزا، بهترین قیمت | اقساطی',
          text: 'آژانس مسافرتی آمیتیس، همراه مطمئن شما در سفر. خدمات اخذ ویزا، رزرو تورهای داخلی و خارجی و برنامه‌ریزی سفرهای خاص. تجربه‌ای متفاوت از سفر را با ما تجربه کنید. برای اطلاعات بیشتر و مشاوره رایگان، به وب‌سایت ما مراجعه کنید.',
      ...
      alert('مرورگر شما از اشتراک‌گذاری نیتیو پشتیبانی نمی‌کند.');
    }
  });
  ...
  function showMoreDates(element , dropdownid){
    ...
    dropDown.classList.toggle("h-28");
    rotateicon.classList.toggle("rotate-180");

    console.log(btnText);
    if(btnText == 'مشاهده بیشتر'){
        console.log("کمتر")
        element.querySelector(".dropdown-txt").innerText = 'مشاهده کمتر';
    }else{
        console.log("بیشتر")
        element.querySelector(".dropdown-txt").innerText = 'مشاهده بیشتر';
    }
}
...
// updated (align logic with desktop; keep your current SVGs per type)
const renderPathSvg = async (element) => {
  try {
    if (!element) return '';
    if (element.info.transportation.type == 1) {
      return `<!-- SVG type 1 (همین کدی که الان در موبایل داری) -->`;
    } else if (element.info.transportation.type == 2) {
      return `<!-- SVG type 2 -->`;
    } else if (element.info.transportation.type == 3) {
      return `<!-- SVG type 3 -->`;
    } else {
      return `<!-- SVG default/other -->`;
    }
  } catch (err) {
    console.error('renderPathSvg=' + err.lineNumber + ',' + err.message);
    return '';
    ...
    if (element) {
      return `<svg width="30" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd"
          d="M11 20C6.03023 20 2 15.9698 2 11C2 6.03023 6.03023 2 11 2C15.9698 2 20 6.03023 20 11C20 15.9698 15.9698 20 11 20ZM11 5.14603C13.462 5.14603 15.4578 7.14183 15.4578 9.60387C15.4578 12.0658 11 16.8539 11 16.8539C11 16.8539 6.54217 12.0658 6.54217 9.60387C6.54217 7.14183 8.53797 5.14603 11 5.14603ZM11 7.83271C11.9334 7.83271 12.6904 8.5897 12.6904 9.52322C12.6904 10.4567 11.9334 11.2137 11 11.2137C10.0666 11.2137 9.30956 10.4567 9.30956 9.52322C9.30956 8.5897 10.0666 7.83271 11 7.83271Z"
          fill="${document.querySelector(".layout__body__container").dataset.primarycolor}"></path>
      </svg>`;
    }
  } catch (err) {
...
const renderHotels = async (element, type) => {
    try {
        if (!element?.hotelinfo?.[0]?.hotels?.length) return "";
        ...
        for (let index = 0; index < hotels.length; index++) {
            ...
            outputs.push(`
                <div class="w-[95%] mx-auto mt-2 flex gap-x-2 border-b border-primary-100 h-28">



                <div class="flex gap-x-1 absolute left-0 -top-2">
                    <div class="relative w-[16px] h-[48px] flex items-center justify-center"><svg class="block opendropdow absolute inset-0" width="16" height="48">
                    <use href="./images/sprite-icons.svg#label-off"></use>
                    </svg><span class="text-[8px] text-white font-yekanbakhsemiboldFA writing-mode-sideways-lr 
                    absolute inset-0 flex items-center justify-right pt-1">${hotel?.label}
                    </span></div>
                </div>
                    ...
                    <figure data-index="${index}" class="relative min-w-24 min-h-24 max-h-24 h-24 rounded-r-2xl rounded-l-[4px] overflow-hidden border">
                        <img class="tourInventory__details__item__img w-24 h-24 max-h-24 object-cover"
                             src="${hotelImg}"
        ...
        data-basiscoreid="${hotel.basiscoreid}" 
                    ...
                    <div class="w-full pl-3">
                        <div class="h-18 pt-2 flex dir-ltr flex-col">
                            <div class="flex mb-2 tourInventory__details__item__rate" data-value="${hotel.star || 0}">
                                ${await renderHotelRate(hotel)}
                            </div>
                            <h2 class="showhotel text-left font-yekanbakhsemiboldFA text-sm leading-4 -tracking-wide line-clamp-2 ">
                                ${escapeHtml(cleanHotelName)}
                            </h2>
                        </div>

                        <div class="dir-ltr flex justify-between flex-wrap font-yekanbakhregularFA text-xs
                                    text-txtneutral-900 min-h-8 my-2 items-center gap-x-1">
                            ...
                            <div class="w-[40%] flex items-center gap-1">
                                <svg class="inline-block" width="16" height="16" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <use href="./images/sprite-icons.svg#tour-icon-dinner"></use>
                                </svg>
                                <span class="uppercase tourInventory__details__item__service" data-value="${hotel.service.vid}">
