  ...
  if (window.innerWidth > 1024) {
    document.addEventListener("click", function (event) {
      if (event.target.closest("#multi")) {
        ...
        const addDestinationBtn = document.querySelector(
...
if (window.innerWidth > 1024) {
  function closeAllMenus() {
    document
      .querySelectorAll(".megamenu-box, .dropdown-menu")
      .forEach((menu) => {
  ...
  document
    .getElementById("megamenu-amitistour")
    .addEventListener("click", function (event) {
      if (event.target === this) {
        ...
        this.closest("li")
          .querySelector(".arrowkey-main-navitem")
          .classList.remove("rotate-180");
        this.closest("li")
          .querySelector(".arrowkey-main-navitem path")
          .classList.remove("fill-secondary");
      }
    });

  function openSubMegaMenu(
    submenu,
    submegamenuclass,
    displaymode,
    siblingsclass
  ) {
    let siblings = submenu
      .closest(".megamenu-box")
      .querySelectorAll("." + siblingsclass);
  ...
  function ToggleFAQ(element) {
    document.querySelectorAll(".faq-active").forEach((activeItem) => {
      if (activeItem !== element) {
        ...
        let numberText = numberContainer.querySelector(
          ".number-of-faq-row div"
        );
...
function ShareSocialMediaMob(event, containerid) {
  ...
  if (isOpen) {
    ...
    document.querySelectorAll(".share-container").forEach((el) => {
      ...
      if (bg)
        bg.classList.remove("right-0", "!mx-0", "h-full", "w-full", "p-0");
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // اگر دکمه پخش وجود دارد => اجرای WaveSurfer
  if (playPauseBtn && audioElement) {
    wavesurfer = WaveSurfer.create({
      ...
      cursorColor: "transparent",
      barWidth: 1,
      height: window.innerWidth > 1024 ? 40 : 10,
      responsive: true,
    });
    ...
    function togglePlayPause() {
      if (wavesurfer.isPlaying()) {
        ...
        document
          ...
          .classList.remove("active-playPause");
      } else {
        // توقف ویدیو در صورت وجود
        const iframe = document.querySelector("#videoContainer iframe");
        if (iframe) {
        ...
        wavesurfer.play();
        waveContainer.classList.add("playing");
        document.querySelector(".playPause").classList.add("active-playPause");
      }
    }
  ...
  // اجرای Swiper اگر passengers-video وجود دارد
  ...
  // اگر یکی از سه آیتم ویدیویی وجود داشته باشد، اجرای مودال
  if (
    document.querySelector(".passengers-video") ||
    document.getElementById("podcastandvideo") ||
    document.getElementById("articles-video")
  ) {
        ...
        if (videoId) {
          try {
            const response = await fetch(
              `load-items.bc?id=${videoId}&typecard=video`
            );
            ...
            if (embedCode.startsWith("http")) {
              const iframeHTML =
                window.innerWidth > 1024
                  ...
                  : `<iframe class="absolute inset-0 w-full h-full object-contain" src="${embedCode}" allowfullscreen frameborder="0"></iframe>`;

              videoContainer.innerHTML = iframeHTML;
              videoModal.classList.remove("hidden");

              // توقف صوت هنگام باز کردن مودال
  ...
  items.forEach((item) => {
    ...
    header.addEventListener("click", function () {
      ...
      items.forEach((el) => {
        ...
        if (window.innerWidth < 1024) {
          el.querySelector("div.border").classList.add(
            "invisible",
            "h-0",
            "p-0",
            "opacity-0"
          );
          el.querySelector("div.border").classList.remove(
            "visible",
            "h-fit",
            "py-3",
            "px-3",
        ...
        el.querySelector(".arrow-visa-documents").classList.remove(
          "rotate-180"
        );
        el.querySelector("h2 span").classList.remove("text-primary");
        el.querySelector(".flex").classList.remove("bg-primary-50");
        el.querySelector(".arrow-visa-documents path").classList.remove(
          "fill-primary"
        );
...
function OpenHamburgerMenu(element, menuContainer) {
  ...
  element
    .closest("div")
    .querySelector(".hamburger-menu-closed")
    .classList.toggle("hidden");
  element
    .closest("div")
    .querySelector(".hamburger-menu-opened")
    .classList.toggle("hidden");
...
function openSubMenu(element) {
  element
    .closest("li")
    .querySelector(".hamburgermenu-submenu")
    .classList.toggle("hidden");
...
const activeTab = Object.entries(activeMap).find(([cls]) =>
  bodyClass.includes(cls)
)?.[1];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (checkboxes.length > 0) {
    checkboxes.forEach((checkbox, index) => {
      ...
      if (index === 0) {
        ...
        fetch(`load-items.bc?catid=${categoryId}&typeid=${typeid}`, {
          ...
          .then((data) => {
            ...
            let scripts = itemWrapper.querySelectorAll("script");
      ...
      checkbox.addEventListener("change", () => {
        if (checkbox.checked) {
          ...
          const item = resultContainer.querySelector(
            `[data-id="${categoryId}"]`
          );
          if (item) {
            item.remove();
    ...
    const maincategoryId = document
      .querySelector("#resultContainer-mainlist")
      .getAttribute("catid");
    const maintypeid = document
      .querySelector("#resultContainer-mainlist")
      .getAttribute("typeid");
    const resultContainermain = document.querySelector(
      "#resultContainer-mainlist"
    );
  ...
  if (searchForm) {
    searchForm.addEventListener("submit", (e) => {
      ...
      const searchCatid = searchInput.closest("form").querySelector(".catid-search").value;
      if (searchText.trim() !== "") {
        resultContainer.innerHTML = ""; // پاک کردن نتایج قبلی
        fetch(
          `load-items.bc?q=${encodeURIComponent(
            searchText
          )}&searchtype=${searchType}&catid=${searchCatid}`,
          {
            method: "GET",
          }
        )
...
document.addEventListener("DOMContentLoaded", function () {
  const breadcrumbList = document.querySelector(".breadcrumbs-list");
...
const scrollToTourSection = async (element, type) => {
  try {
    window.scroll({
      top: document.querySelector(`#${type}`).offsetTop - 100,
      behavior: "smooth",
    });
  } catch (err) {
...
document.addEventListener("DOMContentLoaded", function () {
  if (document.querySelector(".destination-items")) {
    const destinationItems = document.querySelectorAll(
      ".destination-items article"
    );
    const destinationsData = document.querySelectorAll(".each-destinations");
    destinationsData.forEach((dataDiv) => {
      const catInput = dataDiv.querySelector(".destinations-cat");
      const picInput = dataDiv.querySelector(".destinations-pic");
      if (!catInput || !picInput) return;
      const catValue = catInput.value.trim();
      picInput.value = picInput.value.replace(/<\/?p>/g, "");
      destinationItems.forEach((article) => {
        const data_cat = article.getAttribute("data-cat");
        if (data_cat === catValue) {
          const img = article.querySelector("img");
          if (img) {
            const src = img.getAttribute("data-src");
            if (img.src) {
              const srcParts = src.split("/");
              srcParts[4] = picInput.value;
              img.setAttribute("data-SRC", srcParts.join("/"));
            }
          }
        }
      });
...
function show_contact_opt(e) {
  e.closest(".each-contact-opt")
    .querySelector(".contact-opt")
    .classList.toggle("hidden");
...
if (document.getElementById("tour-services")) {
  if (document.querySelector(".tour-service-item")) {
    document.querySelectorAll(".tour-service-item").forEach((item) => {
      const title = item.querySelector(".tour-service-tlt");
      if (title && title.innerText.includes("بیمه")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-insurance");
        }
      }
      if (title && title.innerText.includes("راهنما تور")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-guide");
        }
      }
      if (title && title.innerText.includes("پرداخت")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-dollar");
        }
      }
      if (title && title.innerText.includes("ویزا")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-visa");
        }
      }
      if (title && title.innerText.includes("صبحانه")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-coffee");
        }
      }
      if (title && title.innerText.includes("ناهار")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-reserve");
        }
      }
      if (title && title.innerText.includes("شام")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-reserve2");
        }
      }
      if (title && title.innerText.includes("سه وعده")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-reserve3");
        }
      }
      if (title && title.innerText.includes("پرواز")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-ticket");
        }
      }
      if (title && title.innerText.includes("کنسرت")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-microphone");
        }
      }

      if (title && title.innerText.includes("جشن")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-celebration");
        }
      }

      if (title && title.innerText.includes("آپشینال")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-ship");
        }
      }

      if (title && title.innerText.includes("بازی")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-ticket2");
        }
      }

      if (title && title.innerText.includes("شاد")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-celebration2");
        }
      }

      if (title && title.innerText.includes("ترانسفر فرودگاهی")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-car");
        }
      }

      if (title && title.innerText.includes("ترانسفر نمایشگاهی")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-car");
        }
      ...
      if (title && title.innerText.includes("ترانسفر بین شهری")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-car");
        }
      ...
      if (title && title.innerText.includes("ترانسفر هوایی")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-sea");
        }
      }
      if (title && title.innerText.includes("سیم کارت رایگان")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-simcard");
        }
      }
      if (title && title.innerText.includes("سافاری رایگان")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-discover");
        }
      }
      if (title && title.innerText.includes("قطار")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-train");
        }
      }
      if (title && title.innerText.includes("گشت خرید")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-bag2");
        }
      }
      if (title && title.innerText.includes("گشت شهری")) {
        const icon = item.querySelector(".tour-service-icon");
        if (icon) {
          icon.classList.add("service-routing");
        }
      }
    });
  }
...
if (
  document.querySelector(".most-visited-section") &&
  ...
  function updateCards() {
    cards.forEach((card, index) => {
      ...
      if (offset < 0) {
        ...
        card.style.transform = `translateX(${offset * -20}px) scale(${
          1 - offset * 0.05
        })`;
      }
    });
...
function showAudioBox() {
  document.getElementById("fixedAudioBox").classList.remove("hidden");
  document.getElementById("fixedAudioBox").classList.add("visible");
}
function closeAudioBox() {
  document.getElementById("fixedAudioBox").classList.add("hidden");
  document.getElementById("fixedAudioBox").classList.remove("visible");
}
if (
  document.querySelector(".audio-extra-tour") &&
  document.querySelector(".tour-list-name")
) {
  const tour_list_name = document.querySelector(".tour-list-name").innerText;
  document.querySelector(".audio-extra-tour").innerText = tour_list_name;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  openMenu?.addEventListener("click", function () {
    const hamburgerBtn = document.querySelector(".hamburger-menu-closed");
    const TourBtn = document.querySelector(".open-tours");
    if (hamburgerBtn) {
      OpenHamburgerMenu(hamburgerBtn, ".hamburger-menu");
    }
    if (TourBtn) {
      openSubMenu(TourBtn);
    }
...
document.addEventListener("DOMContentLoaded", () => {
  const span = document.querySelector(".advertise-banner span.hidden");
  if (span && (!span.textContent || span.textContent.trim() === "")) {
    const parent = span.closest(".advertise-banner");
    if (parent) {
