...
const scrollToTourSection = (element, type) => {
    ...
    const firstScrollTop = targetSection.getBoundingClientRect().top + window.scrollY;
    ...
    setTimeout(() => {
        ...
        const accurateTop = targetSection.getBoundingClientRect().top + window.scrollY - offset;
    ...
    try {
        if (element) {
            ...
            if (page_lang === 'fa') {
                ...
                const gDate = JalaliDate.jalaliToGregorian(parseInt(date[0]), parseInt(date[1]), parseInt(date[2]));
...
const onrenderedInventoryView = async () => {
    try {
        if (document.querySelectorAll(".tourInventory__details__item__img").length === 0) {
            const newHtml = `
...
const renderHotels = async (element, type) => {
    try {
        if (element) {
            ...
            for (const [index, item] of hotels.entries()) {
                ...
                let borderClass = index === hotels.length - 1 ? "" : "border-b border-dashed border-module-gray-2 mb-4 pb-4";
                output += `
    ...
    <figure class="shrink-0 relative rounded-xl max-sm:mx-auto overflow-hidden">
        ...
        data-id="${item.hotel.hotelname1.hotelid}"
        data-basiscoreid="${item.hotel.basiscoreid}" 
    ...
    <figcaption class="md:w-9/12">
        <div class="md:flex justify-between max-md:text-center gap-8">
            <div class="md:w-2/4 text-module-gray-6">
                ...
                ${item.hotel.location ? `
                <div class="mt-2">
                    <a href="${item.hotel.location.replace(/"/g, '"').replace(/&/g, '&')}" 
                       target="_blank" 
                       class="text-sm text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1">
                        ...
                        ${page_lang === 'fa' ? 'نمایش روی نقشه' :
                            page_lang === 'en' ? 'View on Map' :
                                page_lang === 'ar' ? 'عرض على الخريطة' : 'View on Map'}
                    </a>
                </div>` : ''}
                ${item.hotel.description ? `<div class="text-sm mt-2">${item.hotel.description}</div>` : ``}
                ${await renderHotelBooking(item.hotel.booking)}
            </div>

            <div class="md:w-2/4 md:ml-auto md:text-left">
...
const renderTourForm = async (element) => {
    ...
    if (element.closest(".tourInventory__details__item__info")) {
...
const onrenderedSchemaTourBookingForm = async (args) => {
    try {
        ...
        const setValue = (selectorInput, selectorText) => {
            const inputEl = container.querySelector(selectorInput)?.querySelector("input");
            const textEl = container.querySelector(selectorText);
            if (inputEl && textEl) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const breadcrumbContainer = document.querySelector(".breadcrumb__container ul");
...
function checkToggleContent(element, parentSelector, maxLines = 3, lineHeightPx = 24) {
    ...
    if (scrollHeight <= maxHeight) {
        ...
        element.innerText = page_lang === 'fa' ? 'مشاهده بیشتر' : page_lang === 'ar' ? 'عرض المزيد' : 'View more';
    }
}
...
const toggleContent = (element, parentSelector, style) => {
    try {
        ...
        const hiddenOnly = /tourExecution__container__(origins|destinations)/.test(pSel);
        if (hiddenOnly) {
        ...
        if (['svg', 'path'].includes(element.tagName?.toLowerCase()))
            toggleTextEl = container.querySelector('.toggle__text, .tourItinerary__item__more__link');

        const lineHeightPx = 24, maxHeight = lineHeightPx * 3;
        const currentMax = content.style.maxHeight || getComputedStyle(content).maxHeight;
        const isCollapsed = currentMax !== 'none' && currentMax !== '' && currentMax !== '0px';

        if (isCollapsed) {
            content.style.maxHeight = 'none';
            content.classList.remove('overflow-hidden');
            if (toggleTextEl) {
                const lang = (typeof page_lang === 'string' ? page_lang : 'en').toLowerCase();
                toggleTextEl.innerText = lang === 'fa' ? 'مشاهده کمتر' : lang === 'ar' ? 'عرض أقل' : 'View less';
            }
        } else {
            content.style.maxHeight = maxHeight + 'px';
            content.classList.add('overflow-hidden');
            if (toggleTextEl) {
                const lang = (typeof page_lang === 'string' ? page_lang : 'en').toLowerCase();
                toggleTextEl.innerText = lang === 'fa' ? 'مشاهده بیشتر' : lang === 'ar' ? 'عرض المزيد' : 'View more';
            }
        }
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.tourDescription__container').forEach(container => {
